...
function get_apmstuff() { # get_apmstuff file (path)
	...
	REPOSITORY_URL="https://github.com/how2secuerity/MiSTer_Player"
...
function apm_update() { # apm_update (next command)
	...
	if [ ! "$(dirname -- ${0})" == "/tmp" ]; then
		...
		# Download the newest MiSTer_Player_on.sh to /tmp
		get_apmstuff MiSTer_Player_on.sh /tmp
		if [ -f /tmp/MiSTer_Player_on.sh ]; then
			if [ ${1} ]; then
				echo " Continuing setup with latest"
				echo " MiSTer_Player_on.sh..."
				/tmp/MiSTer_Player_on.sh ${1}
				exit 0
			fi
		else
			# /tmp/MiSTer_Player_on.sh isn't there!
			echo " APM update FAILED"
			echo " No Internet?"
			exit 1
		fi
	else # We're running from /tmp - download dependencies and procced
		cp --force "/tmp/MiSTer_Player_on.sh" "/media/fat/Scripts/MiSTer_Player_on.sh"
		get_apmstuff .MiSTer_APM/MiSTer_APM_init
		get_apmstuff .MiSTer_APM/MiSTer_APM_MCP
		get_apmstuff .MiSTer_APM/MiSTer_APM_Joy.py 
		get_apmstuff .MiSTer_APM/MiSTer_APM_Keyboard.sh 
		get_apmstuff .MiSTer_APM/MiSTer_APM_Mouse.sh 
